...
def sample():
    ...
    sp_images = {'Sample 1': "\Images\heeelo.png",
                 'Sample 2': '\Images\heelo.png',
                 'Sample 3': '\Images\helo.png'}
    ...
    for aadit, sp in enumerate(sp_images.values()):  # loop to generate caption and hashtags for the sample images

        if next(colms).button("Generate", key=aadit):  # Prediction is called only on the selected image
            ...
            for mem in hashtags:  # Present Hashtags
                st.write(mem)
